...
def read_fasta_alignment(filename):
    ...
    #ADDED LINE
    alignment = [seq for seq in alignment if len(seq) > 0]
    #ADDED LINE END
...
for opt, arg in opts:
    ...
    if opt == "-o":
        ...
        elif arg == 'vn_entropy': scoring_function = vn_entropy
...
for i, score in enumerate(scores):
    if seq_specific_output:
        ...
        if outfile_name == "":
            ...
            outfile.write("%d,%s,%5f," % (i, cur_aa, score))
    else:
        if outfile_name == "":
...
print("Results written to conservation.csv")